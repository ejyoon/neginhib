...
To explore whether these parameters change significantly throughout these early years, we focused just on children's data and analyzed age group as a continuous variable.  This analysis revealed a marginally significant increase in drift rate across these three years ($\beta = .14$, $p = .05$), as well as a marginally significant decrease in non-decision time ($\beta = -.03$, $p = .08$).  These findings indicate that the speed at which children accumulate evidence and the time it takes children to process basic stimuli-level information develops rapidly across early childhood.  
